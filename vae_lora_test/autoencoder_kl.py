...
class AutoencoderKL(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        encoder_attn_type: str = 'lora',
        decoder_attn_type: str = 'lora',
    ):
        super().__init__()
        ...
        self.encoder = Encoder(
            ...
            double_z=True,
            attention_processor = encoder_attn_type,
        )
        ...
        if encoder_attn_type == 'lora' or decoder_attn_type == 'lora':
            self.attention_processor = 'lora'
        else:
    ...
    @apply_forward_hook
    def encode(self, x: torch.FloatTensor, return_dict: bool = True, lora_scale : float = 1.0 ) -> AutoencoderKLOutput:
        # self.use_tiling 在 init 中預設為 False
        # => 需要以 enable_tiling 開啟
        # tiled encoding mode 是讓較大張的 feature map 可以分開編碼，以節省資源用的
        if self.use_tiling and (x.shape[-1] > self.tile_sample_min_size or x.shape[-2] > self.tile_sample_min_size):
            return self.tiled_encode(x, return_dict=return_dict)
        
        if self.attention_processor == 'lora':
            h = self.encoder( x, lora_scale = lora_scale )
        else:
            print( "autoencoder_kl line-239: LoRA layers are not provided." )
            h = self.encoder( x )

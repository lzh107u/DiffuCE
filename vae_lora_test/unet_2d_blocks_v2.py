...
from .attention_processor import Attention, AttnAddedKVProcessor, AttnAddedKVProcessor2_0, LoRAAttnProcessor, AttnProcessor
...
class UNetMidBlock2D(nn.Module):
    ...
    def __init__(
        ...
        output_scale_factor: float = 1.0,
        attention_processor: str = 'lora',
    ):
        """
        UNetMidBlock2D:
        AutoEncoderKL 中，Encoder 的最後一段 & Decoder 的第一段

        注意：UNetMidBlock2D 的 forward 需要被更動，以此來調控 LoRA 提供的引導大小

        Args:
        ----------
        attention_processor: str
        要使用的 AttnProcessor ，可選擇的選項來自 attention_processor.py 內的各種實作
        目前這裡支援以下選擇：
        1. "lora" : LoRAAttnProcessor
        2. else : AttnProcessor
        ...
        for _ in range(num_layers):
            if self.add_attention:
                if attention_processor == 'lora':
                    attn_processor = LoRAAttnProcessor( hidden_size = in_channels )
                else:
                    attn_processor = None
                attentions.append(
                    Attention(
    ...
    def forward(
            ...
            scale: Optional[float] = 0.8 ) -> torch.Tensor:
        hidden_states = self.resnets[0](hidden_states, temb)
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if attn is not None:
                # hidden_states = attn(hidden_states, temb=temb)
                # 將 Attn 操作分成兩個 case ：有 LoRA 與沒有 LoRA
                # 這裡指定 LoRA 用 case2 ，其餘都用 case1
                # 最早 case1 使用的條件是 if isinstance( attn.processor, AttnProcessor )
                # 但有可能在後續版本中被 AttnProcessor_2_0 取代，因此用 not LoRA 比較沒有兼容性的問題
                if not isinstance( attn.processor, LoRAAttnProcessor ):
                    hidden_states = attn( hidden_states, temb = temb )
                elif isinstance( attn.processor, LoRAAttnProcessor ):
                    hidden_states = attn(hidden_states, temb=temb, scale = scale )
                else:
                    raise ValueError( "No match attention processor !" )
